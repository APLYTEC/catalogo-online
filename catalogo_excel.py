...
if familia_actual:
    ...
    if st.session_state.subfamilia_actual is None:
        busqueda_familia = st.text_input(
            "Busca por nombre o código dentro de esta familia",
            key=f"busqueda_familia_{familia_actual}"
        )

        if busqueda_familia:
            productos_familia = df[df["Familia"] == familia_actual].copy()
            productos_familia = productos_familia[
                productos_familia["Nombre"].str.contains(busqueda_familia, case=False, na=False) |
                productos_familia["Código"].astype(str).str.contains(busqueda_familia, case=False, na=False)
            ]
            ...
            for _, fila in productos_familia.iterrows():
                ...
                with c2:
                    ...
                    with a1:
                        cantidad = st.number_input(
                            f"Cantidad familia {fila['Código']}",
                            min_value=1,
                            max_value=1000,
                            value=1,
                            key=f"cantidad_fam_{fila['Código']}"
                        )
                    with a2:
                        tipo = st.selectbox("Formato", FORMATOS, key=f"tipo_fam_{fila['Código']}")
                    with a3:
                        if st.button("➕ Añadir al pedido", key=f"add_fam_{fila['Código']}", use_container_width=True):
                            existente = None
                            for item in st.session_state.carrito:
                                if item["Código"] == fila["Código"] and item["Tipo"] == tipo:
                                    existente = item
...
if familia_actual and subfamilia_actual:
    busqueda = st.text_input("Buscar dentro de esta subfamilia", key=f"busq_sub_{familia_actual}_{subfamilia_actual}")
