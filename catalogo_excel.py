...
GOOGLE_SHEET_ID = "1-lRMkBoIiu0o61Hoex-ecjbuwChOABD8JV-yLC9qRAQ"
...
def normalizar_telefono(telefono):
    ...
    if digitos.startswith("0034") and len(digitos) >= 13:
        digitos = digitos[4:]
...
def obtener_hoja_pedidos():
    ...
    try:
        ...
        return cliente.open_by_key(GOOGLE_SHEET_ID).worksheet(GOOGLE_SHEET_TAB)
    except Exception:
        return None


def guardar_pedido_en_google_sheets(nombre, telefono, carrito, total, comentarios):
    hoja = obtener_hoja_pedidos()
    if hoja is None:
        return False, "Google Sheets no está configurado todavía."

    telefono_norm = normalizar_telefono(telefono)
    pedido_id = f"APL-{datetime.now(ZoneInfo('Europe/Madrid')).strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:6].upper()}"
    fecha = datetime.now(ZoneInfo("Europe/Madrid")).strftime("%d/%m/%Y %H:%M:%S")
    filas = []
    for item in carrito:
        filas.append([
            fecha,
            ...
            str(item.get("Código", "")),
            str(item.get("Nombre", "")),
            str(item.get("Tipo", "unidades")),
            int(item.get("Cantidad", 0)),
            float(item.get("PrecioUnitario", 0.0)),
            float(total),
            str(comentarios or ""),
        ])
    try:
...
def recuperar_ultimo_pedido_google_sheets(telefono, df):
    hoja = obtener_hoja_pedidos()
    if hoja is None:
        return None, "Google Sheets no está configurado todavía."

    tel = normalizar_telefono(telefono)
    if len(tel) < 9:
        return None, "Introduce un teléfono válido de 9 dígitos."

    try:
        valores = hoja.get_all_values()
    except Exception as exc:
        return None, f"No se ha podido consultar el historial: {exc}"

    if len(valores) < 2:
        return None, "No hay pedidos guardados todavía."

    cab = valores[0]
    idx = {nombre: i for i, nombre in enumerate(cab)}
    oblig = ["PedidoID", "Telefono", "Nombre", "Codigo", "Producto", "Formato", "Cantidad"]
    if any(c not in idx for c in oblig):
        return None, "La hoja de pedidos no tiene el formato esperado."

    filas_tel = [f for f in valores[1:] if len(f) > idx["Telefono"] and normalizar_telefono(f[idx["Telefono"]]) == tel]
    if not filas_tel:
        return None, "No encontramos ningún pedido anterior asociado a ese teléfono."

    ultimo_id = filas_tel[-1][idx["PedidoID"]]
    filas = [f for f in filas_tel if len(f) > idx["PedidoID"] and f[idx["PedidoID"]] == ultimo_id]
    if not filas:
        return None, "No se ha podido reconstruir el último pedido."

    nombre_cliente = filas[0][idx["Nombre"]] if len(filas[0]) > idx["Nombre"] else ""
    carrito = []
    next_id = 1
    codigos_df = {str(row["Código"]).strip(): row for _, row in df.iterrows()}
    faltantes = []

    for f in filas:
        codigo = str(f[idx["Codigo"]]).strip()
        if codigo not in codigos_df:
            faltantes.append(codigo)
            continue
        fila_actual = codigos_df[codigo]
        try:
            cantidad = int(float(str(f[idx["Cantidad"]]).replace(",", ".")))
        except Exception:
            cantidad = 1
        formato = f[idx["Formato"]] if len(f) > idx["Formato"] and f[idx["Formato"]] in FORMATOS else "unidades"
        precio_actual = float(fila_actual["Precio"]) * (1 + IVA)
        carrito.append({
            "id": next_id,
            "Código": codigo,
            "Nombre": str(fila_actual["Nombre"]),
            "Cantidad": max(1, cantidad),
            "Tipo": formato,
            "PrecioUnitario": precio_actual,
        })
        next_id += 1

    if not carrito:
        return None, "Los productos de ese pedido ya no están disponibles en el catálogo actual."
...
def render_inicio():
    ...
    st.markdown("### 🔁 Repetir último pedido")
    st.caption("Introduce el teléfono usado en tu último pedido. Recuperaremos los mismos artículos con los precios actuales.")
    col_tel, col_btn = st.columns([2.2, 1])
    with col_tel:
        tel_repetir = st.text_input("Teléfono del último pedido", key="telefono_repetir_inicio", placeholder="Ej. 647936356", label_visibility="collapsed")
    with col_btn:
        repetir = st.button("🔁 Recuperar", key="btn_repetir_pedido", use_container_width=True)
    if repetir:
        pedido, error = recuperar_ultimo_pedido_google_sheets(tel_repetir, df)
        if error:
            st.warning(error)
        else:
            st.session_state.carrito = pedido["carrito"]
            st.session_state.next_cart_id = pedido["next_id"]
            st.session_state.pedido_nombre = pedido["nombre"]
            st.session_state.pedido_telefono = pedido["telefono"]
            st.session_state.pedido_enviado_ok = False
            st.session_state.pdf_generado = False
            st.session_state.familia_actual = None
            st.session_state.subfamilia_actual = None
            st.session_state.pantalla_actual = "carrito"
            st.session_state.pedido_repetido_aviso = f"Pedido {pedido['pedido_id']} recuperado correctamente."
            sync_query_params()
            st.rerun()
...
def render_carrito():
    ...
    if st.session_state.get("pedido_enviado_ok", False):
        ...
        if st.session_state.get("pedido_guardado_historial_ok") is True:
            st.info("🔁 Este pedido ha quedado guardado y podrás recuperarlo más adelante con tu teléfono.")
        elif st.session_state.get("pedido_guardado_historial_ok") is False:
            st.warning("El pedido se ha enviado correctamente, pero no se pudo guardar en el historial para repetirlo.")
    ...
    if st.session_state.pop("pedido_repetido_aviso", None):
        st.success("🔁 Último pedido recuperado. Revisa cantidades y formatos antes de enviarlo.")
                ...
                if not nombre_limpio or not telefono_limpio:
                    ...
                    if st.session_state.get("ultimo_envio_hash") == envio_hash:
                        ...
                        try:
                            ...
                            # Guardamos también el pedido en Google Sheets para poder repetirlo
                            # desde cualquier navegador o dispositivo mediante el teléfono.
                            carrito_enviado = [dict(x) for x in st.session_state.carrito]
                            guardado_ok, guardado_info = guardar_pedido_en_google_sheets(
                                nombre_limpio, telefono_limpio, carrito_enviado, total, comentarios
                            )
                            st.session_state.pedido_guardado_historial_ok = guardado_ok
                            st.session_state.pedido_guardado_historial_info = guardado_info
...
if "pedido_guardado_historial_ok" not in st.session_state:
    st.session_state.pedido_guardado_historial_ok = None
if "pedido_guardado_historial_info" not in st.session_state:
    st.session_state.pedido_guardado_historial_info = None
